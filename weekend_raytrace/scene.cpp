...
Scene openmc_book_cover() {
  ...
  for (const auto& cell : openmc::model::cells) {
    const auto& cell_name = cell->name();

    if (cell_name == "ground") {
      objects.add(std::make_shared<OpenMCCell>(cell.get(), ground_mat));
    } else if (cell_name == "Lambertian") {
      Color albedo = Color::random(0, 1);
      auto sphere_material = std::make_shared<Lambertian>(albedo);
      objects.add(std::make_shared<OpenMCCell>(cell.get(), sphere_material));
    } else if (cell_name == "Metal") {
      auto albedo = Color::random(0.5, 1);
      auto fuzz = nrand(0, 0.5);
      auto sphere_material = std::make_shared<Metal>(albedo, fuzz);
      objects.add(std::make_shared<OpenMCCell>(cell.get(), sphere_material));
    } else if (cell_name == "Dielectric") {
      auto sphere_material = std::make_shared<Dielectric>(1.5); // glass
      objects.add(std::make_shared<OpenMCCell>(cell.get(), sphere_material));
    } else {
      throw std::invalid_argument("Bad material found");
    }
  }
